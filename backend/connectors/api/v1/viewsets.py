...
class efergtrynViewSet(BaseConnectorViewSet):
    PASSWORD = settings.CONNECTOR_EFERGTRYN_PASSWORD
    USERNAME = settings.CONNECTOR_EFERGTRYN_USERNAME
    BASE_URL = "https://api.fruitytyy.com"
    AUTH_TYPE = "basic"
    IDENTIFIER = "CONNECTOR_EFERGTRYN"

    @action(detail=False, methods=["get"], url_path="search/tests")
    def search_tests(self, request, *args, **kwargs):
        params = request.query_params
        payload = request.data
        data = self._api_call(
            "/search/tests/", request_type="get", payload=payload, params=params
        )
